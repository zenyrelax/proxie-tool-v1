...
def change(option:str):
    ...
    match option:
        ...
        case "proxy_timeout":
            print(f"    [{red}>{reset}] Pick proxy timeout")
            print("\n")
            timeout = input(f"    [{red}>{reset}] ")
            if not timeout.isdigit(): return
            if int(timeout) <= 0: timeout = 1
            Checker.timeout = int(timeout)
            values["proxy_timeout"] = Checker.timeout
        case "retries":
            print(f"    [{red}>{reset}] Pick max request retries")
            print("\n")
            retries = input(f"    [{red}>{reset}] ")
        ...
        case "threads":
            print(f"    [{red}>{reset}] Pick ammount of threads")
            print("\n")
            threads = input(f"    [{red}>{reset}] ")
        ...
        case 'api_key':
            print(f"    [{red}>{reset}] Input API key")
            print(f"    [{red}>{reset}] Enter nothing to disable")
            print("\n")
            api_key = input(f"    [{red}>{reset}] ")
            values["api_key"] = api_key
        case 'discord_webhook':
            print(f"    [{red}>{reset}] Input discord webhook")
            print(f"    [{red}>{reset}] Enter nothing to disable")
            print("\n")
            discord_webhook = input(f"    [{red}>{reset}] ")
