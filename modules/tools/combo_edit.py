...
def start():
    ...
    print(f"\n\n    [{red}>{reset}] Finished Editing Combo")
    print(f"    [{red}>{reset}] Saved to \"{save_path}\\Combo_Editor.txt\"")
...
def edit(combos):
    ...
    clear()
    ascii()
    print(f"    [{red}Please Wait, Editing Combos{reset}]")
    acc = 0
    for combo in combos:
        acc += 1
        print(f'    [{red}{int((acc/len(combos))*100)}%{reset}]',end='\r')
