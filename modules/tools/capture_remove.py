...
def start():
    ...
    print(f"    [{red}>{reset}] Imported {green}{len(original_combos)}{reset} Combos")
    ...
    print(f"\n\n    [{red}>{reset}] Finished Removing Capture")
    print(f"    [{red}>{reset}] Saved to \"{save_path}\\Capture_Remove.txt\"")
    input(f"    [{red}>{reset}] Press Enter To Go Back")

def edit(combos):
    clear()
    ascii()
    print(f"    [{red}Please Wait, Removing Capture{reset}]")
    combo_number = 0
    to_save = []
    for combo in combos:
        combo_number += 1
        print(f'    [{red}{int((combo_number/len(combos))*100)}%{reset}]',end='\r')
        if not ":" in combo:
            continue
